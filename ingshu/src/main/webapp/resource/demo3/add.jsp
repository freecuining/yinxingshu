...
<body>
	...
	<div style="font-size: 13px; margin: 10px 5px">
		<form action="cnRole/insertRole" method="post"
			enctype="multipart/form-data" target="_right">
			<table border="1" width="100%" class="table_a">
				<tr>
					<td width="120px;">职位编码<span style="color: red">*</span>：
					</td>
					<td><input type="text" name="roleCode" value="${maxId+1 }"
				...
				<tr>
					...
					<td><c:forEach items="${menuList }" var="menuList">
							<c:if test="${menuList.parentMenu == 0 }">
								<input type="checkbox" name="menuId" value="${menuList.menuId }"
									<c:if test="${menuList.menuId == 1 }">checked</c:if> />
					${menuList.menuName }<br>
							</c:if>
							<c:if test="${menuList.parentMenu != 0 }">
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input
									type="checkbox" name="menuId" value="${menuList.menuId }"
									<c:if test="${menuList.menuId == 2 || menuList.menuId == 3 }">checked</c:if> />
							${menuList.menuName }<br>
							</c:if>
						</c:forEach></td>
				</tr>
